...
export default class ServerSettings extends Component {
    ...
    renderSettings(currentServerName) {
        return (
            <ModalBody>
                ...
                <Row className="m-2">
                    Supported Features: {this.getCurrentServerFeatures()}
                </Row>
            </ModalBody>
        );
    }
    ...
    getCurrentServerFeatures() {
        let currentServerFeatures = this.props.serverSettings.serverConfig && this.state.validServer === null ?
        this.props.serverSettings.serverConfig.features : "";
        if (this.state.config && Object.keys(this.state.config).length > 0) {
            currentServerFeatures = this.state.config.features;
        }
        return currentServerFeatures + "";
    }
